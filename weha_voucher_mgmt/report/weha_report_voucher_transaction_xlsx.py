...
class ReportVoucherTransactionHistoryXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
            ...
            if not transaction_types:
                strSQL = """
                    SELECT a.trans_date, d.name as operating_unit_name, 
                        ...
                        a.trans_type, e.voucher_amount, a.source_doc, a.cc_number, a.total_transaction
        ...
        keys = ['No', 'Operating Unit', 'Date', 'Time', 'SKU', 'Name SKU', 'Trx', 'Tid', 'Source Doc', 'Member ID', 'No Kartu', 'Nominal Transaksi', 'Promo', 'No Voucher', 'Trans Type', 'Amount']
        col = 0
        row = 2
        no = 0
        for key in keys:
        ...
        for detail in docs['detail']:
            ...
            sheet.write(row, 8, detail['source_doc'])
            sheet.write(row, 9, detail['member_id'])
            sheet.write(row, 10, detail['cc_number'])
            sheet.write(row, 11, detail['total_transaction'])
            sheet.write(row, 12, detail['promo_name'])
            sheet.write(row, 13, detail['voucher_ean'])
            sheet.write(row, 14, detail['trans_type'])
            sheet.write(row, 15, detail['voucher_amount'])

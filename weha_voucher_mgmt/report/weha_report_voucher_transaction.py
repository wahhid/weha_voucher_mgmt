    ...
    def _get_report_values(self, docids, data=None):
        ...
        if state == 'US':
            if voucher_promo_ids:
                ...
                """.format(state, date_start, date_end, (operating_unit_ids) if isinstance(operating_unit_ids, int) else tuple(operating_unit_ids), (voucher_promo_ids) if isinstance(voucher_promo_ids, int) else tuple(voucher_promo_ids.ids))
            else:
                 strSQL = """
                    SELECT a.trans_date, d.name as operating_unit_name, 
                        b.voucher_sku, e.name as voucher_name,
                    ...
                    ORDER BY d.name, a.trans_date
                """.format(state, date_start, date_end, (operating_unit_ids) if isinstance(operating_unit_ids, int) else tuple(operating_unit_ids))
